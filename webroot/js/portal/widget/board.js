...
Board = function(conf, panel_conf){
    ...
    this.view = new Ext.DataView({
                ...
                /* User owns the message */
                    '<tpl if="this.isOwner(user_id)">',
                        ...
                        '<span class="board-img">',
                            ...
                            '<tpl if="comments_count &gt; 0">',
                                '<span onclick="openCommentWindow(\'Board\',{id})">',
                                    '<img src="js/portal/shared/icons/fam/comment.png" title="View comments" />{comments_count}',
                                '</span>',
                            '</tpl>',
                            '<tpl if="comments_count &lt;= 0">',
                                '<span onclick="openCommentWindow(\'Board\',{id})">',
	    ...
	    store: new Ext.data.JsonStore({
            ...
            fields: ['id','user_id','text','email','created','expire_date','name','surname','comments_count'],
            baseParams: { 
                limit: limit,
